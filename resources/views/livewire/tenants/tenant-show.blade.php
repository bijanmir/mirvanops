<div>
            ...
            <div class="glass-card rounded-2xl p-4 sm:p-6">
                <div class="flex flex-wrap items-start justify-between gap-4 mb-6">
                    <div class="flex items-center gap-4">
                        <div
                            class="w-16 h-16 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center flex-shrink-0">
                            <span
                                class="text-2xl font-bold text-white">{{ substr($tenant->first_name, 0, 1) }}{{ substr($tenant->last_name, 0, 1) }}</span>
                        </div>
                        <div>
                    ...
                    <a href="{{ route('tenants.edit', $tenant) }}"
                        class="btn-secondary px-4 py-2 rounded-lg text-sm font-medium">Edit</a>
                </div>
                    ...
                    @if($tenant->phone)
                        <div class="flex items-center text-sm">
                            ...
                            <a href="tel:{{ $tenant->phone }}"
                                class="text-secondary hover:text-accent">{{ $tenant->phone }}</a>
                        </div>
                    @endif
                    @if($tenant->email)
                        <div class="flex items-center text-sm">
                            ...
                            <a href="mailto:{{ $tenant->email }}"
                                class="text-secondary hover:text-accent">{{ $tenant->email }}</a>
                        </div>
                    @endif
                        ...
                        @foreach($tenant->leases as $lease)
                            <div class="p-4 sm:p-6">
                                <div class="flex items-start justify-between mb-2">
                                    <div>
                                        <h3 class="font-semibold text-primary">{{ $lease->unit->property->name }} - Unit
                                            {{ $lease->unit->unit_number }}</h3>
                                        <p class="text-sm text-muted">{{ $lease->start_date->format('M d, Y') }} -
                                            {{ $lease->end_date->format('M d, Y') }}</p>
                                    </div>
                                ...
                                <p class="text-lg font-bold text-primary">${{ number_format($lease->rent_amount) }}<span
                ...
                @if($tenant->emergency_contact_name)
                    ...
                    @if($tenant->emergency_contact_phone)
                        <a href="tel:{{ $tenant->emergency_contact_phone }}"
                            class="text-sm text-secondary hover:text-accent">{{ $tenant->emergency_contact_phone }}</a>
                    @endif
                @else
    ...
    <div class="mt-6">
        <livewire:document-manager documentable-type="App\Models\Tenant" :documentable-id="$tenant->id" />
    </div>
</div>