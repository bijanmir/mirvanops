<div>
    <!-- Welcome Header -->
    <div class="mb-8">
        <h1 class="text-2xl sm:text-3xl font-bold text-primary">Good
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div class="glass-card rounded-xl p-5">
            <div class="flex items-center justify-between mb-3">
                ...
                <span class="text-xs font-medium text-green-500 bg-green-500/10 px-2 py-1 rounded-full">This
                    Month</span>
            </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        <a href="{{ route('properties.index') }}"
            class="glass-card rounded-xl p-4 hover:border-accent/50 transition-colors group">
            <div class="flex items-center gap-3">
                <div
                    class="w-10 h-10 rounded-lg bg-indigo-500/15 flex items-center justify-center group-hover:bg-indigo-500/25 transition-colors">
        ...
        <a href="{{ route('tenants.index') }}"
            ...
            <div class="flex items-center gap-3">
                <div
                    class="w-10 h-10 rounded-lg bg-purple-500/15 flex items-center justify-center group-hover:bg-purple-500/25 transition-colors">
        ...
        <a href="{{ route('leases.index') }}"
            ...
            <div class="flex items-center gap-3">
                <div
                    class="w-10 h-10 rounded-lg bg-cyan-500/15 flex items-center justify-center group-hover:bg-cyan-500/25 transition-colors">
        ...
        <a href="{{ route('maintenance.index') }}"
            ...
            <div class="flex items-center gap-3">
                <div
                    class="w-10 h-10 rounded-lg bg-rose-500/15 flex items-center justify-center group-hover:bg-rose-500/25 transition-colors">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="glass-card rounded-xl overflow-hidden">
                <div class="p-4 border-b border-primary flex items-center justify-between">
                    ...
                    @if($paymentsDue->count() > 0)
                        <span
                            class="text-xs font-medium text-amber-500 bg-amber-500/10 px-2 py-1 rounded-full">{{ $paymentsDue->count() }}
                            pending</span>
                    @endif
                </div>
                ...
                @if($paymentsDue->count() > 0)
                            ...
                            <div class="p-4 flex items-center justify-between hover:bg-input/50 transition-colors">
                                <div class="flex items-center gap-3">
                                    <div
                                        ...
                                        <span
                                            class="text-sm font-bold text-white">{{ substr($lease->tenant->first_name ?? 'T', 0, 1) }}</span>
                                    </div>
                                    <div>
                                        <p class="text-sm font-medium text-primary">{{ $lease->tenant->full_name ?? 'Unknown' }}
                                        </p>
                                        <p class="text-xs text-muted">{{ $lease->unit->property->name ?? '' }} - Unit
                                            {{ $lease->unit->unit_number ?? '' }}</p>
                                    </div>
                                </div>
                                <div class="flex items-center gap-3">
                                    <div class="text-right">
                                        <p class="text-sm font-bold text-primary">
                                            ${{ number_format($lease->rent_amount + ($lease->pet_rent ?? 0)) }}</p>
                                        <p class="text-xs text-muted">Due
                                            {{ $lease->payment_due_day }}{{ $lease->payment_due_day == 1 ? 'st' : ($lease->payment_due_day == 2 ? 'nd' : ($lease->payment_due_day == 3 ? 'rd' : 'th')) }}
                                        </p>
                                    </div>
                                    <a href="{{ route('payments.create', ['lease_id' => $lease->id]) }}"
                    ...
                    @if($paymentsDue->count() > 5)
                        <div class="p-3 border-t border-primary bg-input/30">
                            <a href="{{ route('payments.index') }}" class="text-sm text-accent hover:underline">View all
                                {{ $paymentsDue->count() }} pending →</a>
                        </div>
                    @endif
                @else
                    ...
                    <div class="divide-y divide-primary">
                        @foreach($recentPayments as $payment)
                            <div class="p-4 flex items-center justify-between hover:bg-input/50 transition-colors">
                                <div class="flex items-center gap-3">
                                    <div
                                        class="w-10 h-10 rounded-lg bg-green-500/15 flex items-center justify-center flex-shrink-0">
                                    ...
                                    <div>
                                        <p class="text-sm font-medium text-primary">
                                            {{ $payment->tenant->full_name ?? 'Unknown' }}</p>
                                        <p class="text-xs text-muted">{{ $payment->period_covered }} •
                                            {{ $payment->payment_method_label }}</p>
                                    </div>
                                </div>
                                <div class="text-right">
                ...
                @else
                    <div class="p-8 text-center">
                        ...
                        <a href="{{ route('payments.create') }}"
                            class="text-sm text-accent hover:underline mt-2 inline-block">Record first payment →</a>
                    </div>
                @endif
            </div>
            ...
            @if($maintenanceAlerts->count() > 0)
                <div class="glass-card rounded-xl overflow-hidden border-l-4 border-red-500">
                    <div class="p-4 border-b border-primary flex items-center justify-between">
                        ...
                        <a href="{{ route('maintenance.index') }}" class="text-sm text-accent hover:underline">View all
                            →</a>
                    </div>
                    ...
                    <div class="divide-y divide-primary">
                        @foreach($maintenanceAlerts as $request)
                            <a href="{{ route('maintenance.show', $request) }}"
                                class="p-4 flex items-center justify-between hover:bg-input/50 transition-colors block">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs text-muted">{{ $request->property->name ?? '' }}@if($request->unit) -
                                        Unit {{ $request->unit->unit_number }}@endif</p>
                                    </div>
                                </div>
        ...
        <div class="space-y-6">
                ...
                @if($expiringLeases->count() > 0)
                    <div class="divide-y divide-primary">
                        @foreach($expiringLeases as $lease)
                            @php
                                $daysLeft = (int) now()->startOfDay()->diffInDays($lease->end_date->startOfDay());
                            @endphp
                        ...
                        @foreach($recentActivity as $log)
                            <div class="p-3 hover:bg-input/50 transition-colors">
                                <div class="flex items-start gap-3">
                                    <div
                                        class="w-8 h-8 rounded-full bg-accent/15 flex items-center justify-center flex-shrink-0 mt-0.5">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <p class="text-xs text-muted">{{ $log->user->name ?? 'System' }} •
            ...
            <div class="glass-card rounded-xl p-4">
                ...
                <div class="space-y-2">
                    <a href="{{ route('payments.create') }}"
                        class="flex items-center gap-3 p-3 rounded-xl bg-green-500/10 hover:bg-green-500/20 transition-colors group">
                        <div
                            class="w-8 h-8 rounded-lg bg-green-500/20 flex items-center justify-center group-hover:bg-green-500/30">
                    ...
                    <a href="{{ route('maintenance.create') }}"
                        class="flex items-center gap-3 p-3 rounded-xl bg-amber-500/10 hover:bg-amber-500/20 transition-colors group">
                        <div
                            class="w-8 h-8 rounded-lg bg-amber-500/20 flex items-center justify-center group-hover:bg-amber-500/30">
                    ...
                    <a href="{{ route('tenants.create') }}"
                        class="flex items-center gap-3 p-3 rounded-xl bg-purple-500/10 hover:bg-purple-500/20 transition-colors group">
                        <div
                            class="w-8 h-8 rounded-lg bg-purple-500/20 flex items-center justify-center group-hover:bg-purple-500/30">
                    ...
                    <a href="{{ route('leases.create') }}"
                        class="flex items-center gap-3 p-3 rounded-xl bg-cyan-500/10 hover:bg-cyan-500/20 transition-colors group">
                        <div
                            class="w-8 h-8 rounded-lg bg-cyan-500/20 flex items-center justify-center group-hover:bg-cyan-500/30">
